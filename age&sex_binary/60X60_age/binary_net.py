...
def train(train_fn,val_fn,
    ...
    for epoch in range(num_epochs):
        ...
        if val_err <= best_val_err:
            ...
            best_epoch_valid = epoch+1
            
            valid_test_err, valid_test_loss = val_epoch(X_test,y_test)
	    ...
	    if save_path is not None:
                np.savez("test"+save_path, *lasagne.layers.get_all_param_values(model))
            #np.savez(save_path_test, *lasagne.layers.get_all_param_values(model))
            #test_err, test_loss = val_epoch(X_test,y_test)
        ...
        print("  best epoch validation:         "+str(best_epoch_valid))
        print("  best validation error rate:    "+str(best_val_err)+"%")
	print("  valid test loss:               "+str(valid_test_loss))
        print("  valid test error rate:         "+str(valid_test_err)+"%") 

        print("  test loss:                     "+str(test_loss))
        print("  test error rate:               "+str(test_err)+"%") 
	print("  best epoch test:               "+str(best_epoch_test))
