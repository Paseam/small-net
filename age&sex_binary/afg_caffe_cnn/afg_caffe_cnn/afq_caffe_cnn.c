...
#ifdef TEST_0_AND_1
#define DIM ((sizeof(SQZTYPE))* 8)
static void cvtbin2bit(int len, int ndim, int * x, SQZTYPE * xx)
{
	int i, j;
	int temp;
	for (i = 0; i < len / ndim; i++)
	{
		for (j = 0; j < ndim; j++)
		{
			temp = (x[i*ndim + j] << j);
	...
	for (j = 0; j < len - len / ndim*ndim; j++)
	{
		xx[i] |= (x[len / ndim*ndim + j] << j);
	}
}

static void cvtbin2bit_mat(int rows, int cols, int ndim, int * x, SQZTYPE * xx, int mode)
{
	int r, c;
	int i, j;
	int *temp_col;
	SQZTYPE * temp_bit;
	MHandle* hMemMgr = 0;
	if (1 == mode)//squeeze by column
	{
		temp_col = (int*)MMemAlloc(hMemMgr, rows*sizeof(int));//new int[rows];
		temp_bit = (SQZTYPE*)MMemAlloc(hMemMgr, sizeof(SQZTYPE)*ceil(1.0*rows / ndim));
		for (c = 0; c < cols; c++)
		{
			for (r = 0; r < rows; r++)
			{
				temp_col[r] = x[r*cols + c];
			}
			memset(temp_bit, 0, sizeof(SQZTYPE)*ceil(1.0*rows / ndim));
			cvtbin2bit(rows, ndim, temp_col, temp_bit);
			for (i = 0; i < ceil(1.0*rows / ndim); i++)
			{
				xx[i*cols + c] = temp_bit[i];
			}
		}
		MMemFree(hMemMgr, temp_col);
		MMemFree(hMemMgr, temp_bit);
	}
	else if (0 == mode)//squeeze by row
	{
		MMemSet(xx, 0, rows*ceil(cols*1.0 / ndim));
		for (r = 0; r < rows; r++)
		{
			cvtbin2bit(cols, ndim, x + r*cols, xx + r*((int)ceil(1.0*cols / ndim)));
		}
	}
}


static void cvtbin2bit_mat_float(int rows, int cols, int ndim, float * x, SQZTYPE * xx, int mode)
{
	int ii = 0;
	int r, c;
	int i, j;
	...
	SQZTYPE * temp_bit;
	MHandle* hMemMgr = 0;
	int *x2 = (int*)MMemAlloc(hMemMgr, rows*cols*sizeof(int));
	MMemSet(x2, 0, rows*cols*sizeof(int));
	double temp = 0;
	int temp_size = ceil(1.0*rows / ndim);
	...
	if (1 == mode)//squeeze by column
	{
		temp_col = (int*)MMemAlloc(hMemMgr, rows*sizeof(int));
		temp_bit = (SQZTYPE*)MMemAlloc(hMemMgr, sizeof(SQZTYPE)*temp_size);
		for (c = 0; c < cols; c++)
		{
			for (r = 0; r < rows; r++)
			{
				temp_col[r] = x2[r*cols + c];
			}
			MMemSet(temp_bit, 0, sizeof(SQZTYPE)*temp_size);
			cvtbin2bit(rows, ndim, temp_col, temp_bit);
			//////////////////////////////////////////////
	...
	{
		MMemSet(xx, 0, rows*ceil(cols*1.0 / ndim)*sizeof(SQZTYPE));
		for (r = 0; r < rows; r++)
		{
			cvtbin2bit(cols, ndim, x2 + r*cols, xx + r*((int)ceil(1.0*cols / ndim)));
		}
	}
	MMemFree(hMemMgr, x2);
}
...
static int afq_mcvMatrixMulMatrixXor_bin(DATATYPE *matrixOut, SQZTYPE* aa, SQZTYPE* bb, int ndim, int row1, int column1, int column2)
{
	int i, j, k;
	SQZTYPE temp;
	unsigned char * ptr;
	int nq = ceil(column1*1.0 / ndim);
	float matrixOut_temp;

	MMemSet(matrixOut, 0, row1*column2*sizeof(DATATYPE));
	...
	for (i = 0; i < row1; i++)
	{
		for (j = 0; j < column2; j++)
		{
			...
			{
#ifdef SQUEEZE_CHAR
				matrixOut_temp += BitsSetTable256[aa[i*nq + k] ^ bb[k*column2 + j]];
#else
				temp = aa[i*nq + k] ^ bb[k*column2 + j];
				ptr = (unsigned char*)(&temp);
				matrixOut_temp += BitsSetTable256[ptr[0]] + BitsSetTable256[ptr[1]] +
					BitsSetTable256[ptr[2]] + BitsSetTable256[ptr[3]];
#endif

			}
			matrixOut[i*column2 + j] = column1 - 2 * matrixOut_temp;
		}
	}
	return 0;
}
...
//static int afq_mcvMatrixMulMatrixXor_bin2(DATATYPE *matrixOut, SQZTYPE* aa, SQZTYPE* bb, int ndim, int row1, int column1, int column2)
//{
//	int i, j, k;
//	DATATYPE *p1;
//	SQZTYPE*p2, *p3;
...
//	unsigned char * ptr;
//	int nq = ceil(column1*1.0 / ndim);
//	DATATYPE* matrixOut_temp = matrixOut;
//	
//	if (matrixOut == 0 || aa == 0 || bb == 0)
//	{
//		return 1;
//	}
...
//				temp= m1 ^ (*bb++);
//				ptr = (unsigned char*)(&temp);
//				(*p1++) -= 2*(BitsSetTable256[ptr[0]] + BitsSetTable256[ptr[1]] +
//					BitsSetTable256[ptr[2]] + BitsSetTable256[ptr[3]]);
//				
//			}
//		}
...
//				temp = (*aa++) ^ (*p2++);
//				ptr = (unsigned char*)(&temp);
//				sum +=  (BitsSetTable256[ptr[0]] + BitsSetTable256[ptr[1]] +
//					BitsSetTable256[ptr[2]] + BitsSetTable256[ptr[3]]);
//				 
//			}
//			(*matrixOut++) -=  (2*sum);
...
//	else
//	{
//		for (i = 0; i < row1; i++, aa += nq, matrixOut += column2)
//		{
//			p2 = bb;
//			for (k = 0; k < nq; k++)
//			{
...
static int afq_mcvMatrixMulMatrixXor_bin2(DATATYPE *matrixOut, SQZTYPE* aa, SQZTYPE* bb, int ndim, int row1, int column1, int column2)
{
	...
	if (1 == row1)
	{
		for (i = 0; i < nq; i++)
		{
			...
			for (j = 0; j < column2; j++)
			{
				temp = m1 ^ (*bb++);
				ptr = (unsigned char*)(&temp);
				(*p1++) += (BitsSetTable256[ptr[0]] + BitsSetTable256[ptr[1]] +
					BitsSetTable256[ptr[2]] + BitsSetTable256[ptr[3]]);
	...
	for (i = 0; i < row1*column2; ++i)
	{
		matrixOut_temp[i] = column1 - 2 * matrixOut_temp[i];
	}
	return 0;
}
...
static int caffecnn_forward_layer_conv(CAFFECNN_LAYER_CONV * convlayer)
{
...
#ifdef TEST_0_AND_1
	SQZTYPE *data_ia_bin_ptr = convlayer->data_ia_bin_ptr;
	SQZTYPE *weight_bin = convlayer->weight_bin;
#endif
	...
	//outputdata = (DATATYPE *)MMemAlloc(0, sizeof(DATATYPE)*top_offset);
	...
	else
	{
		...
		afq_mcvMatrixMulMatrixRowMajor_f32(outputdata, weight, col_data, M_, K_, N_);
#else//TEST_0_AND_1
		cvtbin2bit_mat_float(convlayer->K_, convlayer->N_, DIM, col_data, data_ia_bin_ptr, 1);
		afq_mcvMatrixMulMatrixXor_bin(outputdata, weight_bin, data_ia_bin_ptr,DIM, M_, K_, N_);
...
#ifdef TEST_0_AND_1
	SQZTYPE *data_ia_bin_ptr = fclayer->data_ia_bin_ptr;
	SQZTYPE *weight_bin = fclayer->weight_bin;
#endif
...
#else//TEST_0_AND_1
		MMemSet(data_ia_bin_ptr, 0, sizeof(SQZTYPE)*(ceil(fclayer->input_num*1.0 / DIM)));
		
		cvtbin2bit_mat_float(1, input_num, DIM, inputdata, data_ia_bin_ptr, 0);
		afq_mcvMatrixMulMatrixXor_bin(outputdata, data_ia_bin_ptr, weight_bin,DIM, 1, input_num, output_num);
							 ...
							 MInt32 * cls_result, MFloat * cls_conf, MHandle hMemMgr)
{
	...
	for (c = 0; c < channels; c++)
	{
		for (i = 0; i < height_x; i++)
		{
			for (j = 0; j < width_x; j++)
			{
				//int temp = input_x[i*linebytes_x + j*channels + 2-c];
				net->inputdata[width_x*height_x*c + width_x*i + j] = input_x[i*linebytes_x + j*channels + 2-c];  //input_x[width_x*height_x*c + width_x*i + j];
		...
		case CAFFECNN_LAYER_CODE_CONV:
			{
				...
				//test_0_and_1
				//convlayer->data_ia_bin_ptr = outdataptr;
				...
				//test_0_and_1
#ifdef TEST_0_AND_1
				convlayer->weight_bin = (SQZTYPE*)MMemAlloc(hMemMgr, convlayer->M_*sizeof(SQZTYPE)*(ceil(convlayer->K_*1.0 / DIM)));
				demand_memory += convlayer->M_*sizeof(SQZTYPE)*(ceil(convlayer->K_*1.0 / DIM));
				ASSERT_MEMORY(convlayer->weight_bin);
				MMemSet(convlayer->weight_bin, 0, convlayer->M_*sizeof(SQZTYPE)*(ceil(convlayer->K_*1.0 / DIM)));

				convlayer->data_ia_bin_ptr = (SQZTYPE*)MMemAlloc(hMemMgr, convlayer->N_*sizeof(SQZTYPE)*(ceil(convlayer->K_*1.0 / DIM)));
				demand_memory += convlayer->N_*sizeof(SQZTYPE)*(ceil(convlayer->K_*1.0 / DIM));
				ASSERT_MEMORY(convlayer->data_ia_bin_ptr);
				MMemSet(convlayer->data_ia_bin_ptr, 0, convlayer->N_*sizeof(SQZTYPE)*(ceil(convlayer->K_*1.0 / DIM)));
#endif
		...
		case CAFFECNN_LAYER_CODE_FC:
			{
				...
				//test_0_and_1
#ifdef TEST_0_AND_1
				fclayer->weight_bin = (SQZTYPE*)MMemAlloc(hMemMgr, fclayer->output_num*sizeof(SQZTYPE)*(ceil(fclayer->input_num*1.0 / DIM)));
				demand_memory += fclayer->output_num*sizeof(SQZTYPE)*(ceil(fclayer->input_num*1.0 / DIM));
				ASSERT_MEMORY(fclayer->weight_bin);
				MMemSet(fclayer->weight_bin, 0, fclayer->output_num*sizeof(SQZTYPE)*(ceil(fclayer->input_num*1.0 / DIM)));

				fclayer->data_ia_bin_ptr = (SQZTYPE*)MMemAlloc(hMemMgr, sizeof(SQZTYPE)*(ceil(fclayer->input_num*1.0 / DIM)));
				demand_memory += sizeof(SQZTYPE)*(ceil(fclayer->input_num*1.0 / DIM));
				ASSERT_MEMORY(fclayer->data_ia_bin_ptr);
				MMemSet(fclayer->data_ia_bin_ptr, 0, sizeof(SQZTYPE)*(ceil(fclayer->input_num*1.0 / DIM)));
#endif
	...
	{// load model data
		...
		for (i = 0; i < net->layers_num; i++)
		{
			...
			else if (CAFFECNN_LAYER_CODE_CONV == net->layers_code[i])
			{
				...
				/*DATATYPE* temp_w = (DATATYPE*)MMemAlloc(0, convlayer->K_);
				MMemSet(temp_w, 0, convlayer->K_);
				for (j = 0; j <  convlayer->K_; j++)
				{
					temp_w[j] = pCNNModel[readptr++];
				}*/
				...
				cvtbin2bit_mat_float(convlayer->M_, convlayer->K_, DIM, convlayer->weight, convlayer->weight_bin, 0);
...
#ifdef TEST_0_AND_1
				cvtbin2bit_mat_float(fclayer->input_num, fclayer->output_num , DIM, fclayer->weight, fclayer->weight_bin, 1);
...
#ifdef TEST_0_AND_1
						if (convlayer->weight_bin)
							MMemFree(hMemMgr, convlayer->weight_bin);
						if (convlayer->data_ia_bin_ptr)
							MMemFree(hMemMgr, convlayer->data_ia_bin_ptr);
#endif
...
#ifdef TEST_0_AND_1
						if (fclayer->weight_bin)
							MMemFree(hMemMgr, fclayer->weight_bin);
						if (fclayer->data_ia_bin_ptr)
							MMemFree(hMemMgr, fclayer->data_ia_bin_ptr);
#endif
